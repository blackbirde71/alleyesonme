...
const AttentionProportionChart = ({ studentsAttentionData, duration }) => {
  ...
  useEffect(() => {
    if (studentsAttentionData && d3Container.current) {
      createAttentionProportionChart(studentsAttentionData, duration, d3Container.current);
    }
...
function createAttentionProportionChart(studentsAttentionData, duration, container) {
  ...
  const attentionProportions = Array(timeSteps).fill(0).map((_, index) => {
    const time = index * timeInterval;
    const studentsPayingAttention = studentsAttentionData.filter(studentData => {
      const lastAttentionBefore = studentData.findLast(a => a.timestamp <= time);
      return lastAttentionBefore && lastAttentionBefore.ifLooking;
    });
    return {
      time: time,
      proportion: studentsPayingAttention.length / studentsAttentionData.length
    };
  });
    ...
    if (xValue >= 0 && xValue <= duration) {
      ...
      if (index > 0 && index < attentionProportions.length) {
        ...
        tooltip
          ...
          .html(`Time: ${minutes}:${seconds.toString().padStart(2, '0')}<br>Attention: ${(d.proportion * 100).toFixed(1)}%`)
